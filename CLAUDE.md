...
15. **SEO keywords consolidation + reindex.** The new shared `seo.keywords` field (string[] json on every content collection's `seo` group) supersedes the guides-only `keywords[]`. The `20260611_061406_add_seo_keywords` migration (in `src/migrations/`) adds the column (runs via CI on deploy to `main`); run the backfill **after** it applies.
    ...
    - **Backfill (guides only) — DONE (executed 2026-06-29):** 49 guides' legacy `keywords[]` consolidated into `seo.keywords` (verified: 49/49 populated); 3 guides had no legacy keywords (skipped). Ran from inside `cleanstart-cms-1` via `docker cp` + `pnpm exec tsx scripts/backfill-seo-keywords-from-guides.ts` (the script wasn't in the deployed image yet; `.env` absent — env is via container vars, so NO `--env-file`), with `WEB_REVALIDATE_SUPPRESS=true` to mute the 49 per-doc ISR pings. Idempotent / re-runnable. **The legacy guides `keywords[]` (plus the dead `articleSections[]` and `howTo` group) are now dropped by migration `20260629_120000_drop_guides_legacy_sections_keywords_howto` — that backfill is its prerequisite and is satisfied.** Other content collections never had a legacy `keywords[]`, so this backfill is guides-only and complete.
...
18. **Publish mis-imported Webflow drafts (5 pages, 4 stuck).** Webflow's API reports `isDraft: true` for the *staged* copy of an item, so a page that was live on the source but carried unpublished "changes in draft" exported with `isDraft: true` despite a non-null `lastPublished`. The old `webflowStatus` keyed only on `isDraft`, so those live pages imported as CMS **drafts** and 404 on the new site (soft-404: HTTP 200, generic `<title>` placeholder). The transform is fixed going forward (`apps/cms/src/payload/lib/webflow-import/status.ts` — a non-null `lastPublished` now maps to `published`; jobs' `hiringStatus` is intentionally unchanged); this one-shot publishes the rows already in prod. Affected set (export scan `isDraft===true && lastPublished!=null`): blogs `software-supply-chain-a-practical-guide-for-security-teams`, `container-security-101-concepts-threats-and-best-practices` (stuck), `container-image-signing-enhancing-security-in-the-software-supply-chain` (already published — skipped); guides `attack-surface-reduction-vs-vulnerability-management`, `hardened-container-image` (stuck).

    - Idempotent / re-runnable: an already-published target is skipped; a target not found in prod is logged as `NOT FOUND` (needs a status-corrected re-import of that slug, then re-run). Uses `find({ draft: true, overrideAccess: true })` to surface draft-only rows.
    - Publishing is a genuine first-publish transition, so the publish hooks **do** fire per doc: IndexNow ping (desired), Meilisearch sync (desired), web revalidate (desired), one Teams notification per doc (up to 4), and a version row. **Run in a quiet window** (optionally unset `WEBHOOK_TEAMS_URL` for the invocation).
    - From inside the `cms` container: `pnpm exec tsx --env-file=.env scripts/publish-misimported-webflow-drafts.ts --dry-run` then `pnpm exec tsx --env-file=.env scripts/publish-misimported-webflow-drafts.ts` (no flag).
    - After the run, spot-check the 4 stuck URLs — `/blogs/container-security-101-concepts-threats-and-best-practices`, `/blogs/software-supply-chain-a-practical-guide-for-security-teams`, `/guide/attack-surface-reduction-vs-vulnerability-management`, `/guide/hardened-container-image` should render their real `<title>`/H1 (not the generic `Blog post — CleanStart` / `Guide — CleanStart` placeholder).
19. **Filter→taxonomy promotion seed + backfill (6 taxonomies).** The inline filter enums on Case Studies (`industry`), Resources (`type`), Jobs (`department`), News (`region`, `pressType`) and Webinars (`region`, `webinarType`) were promoted to editor-managed taxonomy collections (`industries`, `resourceTypes`, `departments`, `regions` (shared by News+Webinars), `pressTypes`, `webinarTypes`). Each content collection keeps its legacy enum during the transition and gains a parallel `…Ref` relationship. Full design + the (interactive) migration-generation recipe + per-phase runbook: `docs/superpowers/specs/2026-06-23-promote-filters-to-taxonomies-design.md`.

- **Migrations first.** The 6 taxonomy tables + relationship columns need a Payload migration — `migrate:create` is interactive (no CI/headless path), so generate it in a real terminal against a clean sibling DB per the spec, then it deploys via CI. (Note: the concurrent `deal-registrations` collection is also un-migrated and folds into the same diff unless landed first.)
...
21. **Newsletter subscription opt-in + welcome email (HubSpot ops + env).** Newsletter signups now relay a HubSpot marketing-subscription opt-in (`legalConsentOptions.consent.communications`) keyed off the new `forms.hubspotSubscriptionTypeId` field — so subscribers are actually *subscribed*, not just created as contacts. To make it live: (a) in HubSpot (Settings → Marketing → Email → Subscription Types) create a **"Newsletter"** subscription type and copy its internal id (today only "Marketing Information" + "One to One" exist); (b) set `HUBSPOT_NEWSLETTER_SUBSCRIPTION_TYPE_ID=<id>` in the CMS env (droplet `/opt/cleanstart/.env` + local) and run `scripts/seed-website-forms.ts` (or set `hubspotSubscriptionTypeId` on the `newsletter` form in the admin → Forms) so the relay opts subscribers in; (c) build a HubSpot **workflow** (trigger: Newsletter subscription = subscribed, or newsletter-form submission) that sends a **Welcome** marketing email. Double opt-in stays OFF (single opt-in). The migration `20260623_140000_add_forms_hubspot_subscription_type_id` ships via CI. The web newsletter CTAs now require a visible consent checkbox. Verify: subscribe via a site CTA → the contact shows **subscribed to "Newsletter"** in HubSpot and the welcome workflow fires.
