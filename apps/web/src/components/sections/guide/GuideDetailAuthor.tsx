...
import { guidePickImageUrl } from "@/lib/guides";
import type { GuideAuthor } from "@/lib/guides";

interface GuideDetailAuthorProps {
  authors?: GuideAuthor[] | null | undefined;
}

export function GuideDetailAuthor({
  authors,
}: GuideDetailAuthorProps): React.ReactElement | null {
  if (!authors || authors.length === 0) return null;

  return (
    <section className="relative w-full bg-white" data-section="GuideDetailAuthor">
...
function AuthorName({ author }: { author: GuideAuthor }): React.ReactElement {
...
function AuthorCard({ author }: { author: GuideAuthor }): React.ReactElement {
  const photoSrc = guidePickImageUrl(author.photo, ["card", "hero", "thumb"]);
