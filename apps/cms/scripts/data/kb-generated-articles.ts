// Vendored from the (removed) apps/web kh-articles.data.ts — the 7 legacy
// Knowledge Hub articles imported from the old cleanstart.com Knowledge Hub.
// Consumed by seed-knowledge-base.ts. Do not edit by hand.
type Block =
...
export const GENERATED_ARTICLES: Record<string, Article> = {
  'keyless-container': {
    ...
    title:
      'How to Implement Keyless Container Image Signing Using Sigstore Fulcio and OIDC Identity',
    ...
    blocks: [
      { type: 'heading', text: 'What this article covers' },
      {
        type: 'ul',
        items: [
          'Architecture of Identity-Based Signing: A comprehensive analysis of the interaction between OIDC providers, the Fulcio Root CA, and the Rekor transparency log.',
          'CleanStart Provenance Integration: Methodologies for ensuring that zero-CVE base images retain their chain of custody when used as parent layers.',
          'OIDC Federation Configuration: Detailed instructions for configuring trust relationships in GitHub Actions and GitLab CI/CD pipelines.',
      ...
      {
        type: 'p',
        text: 'Traditional signing relies on asymmetric cryptography where a private key is generated, stored, and protected by the signer. This model fails at scale in containerized environments due to the "secret management problem." If a key is stored in a CI secret manager, it is vulnerable to exfiltration. If it is lost, the artifact cannot be verified. If it is compromised, revocation is complex and rarely enforced effectively. Sigstore solves this through ephemeral signing.',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'Signing provides provenance (who built it) and integrity (it hasn\'t changed), but it does not guarantee quality (it is safe). A signed image containing critical vulnerabilities is merely a verified threat. The CleanStart methodology dictates "upstream thinking," where security is embedded in the foundation.1 Before implementing signing, ensure the Dockerfile references a CleanStart hardened base. These images are stripped of unnecessary components, reducing the attack surface by up to 80% compared to standard Docker Hub images.2',
      },
      { type: 'p', text: 'Correct Dockerfile Structure:' },
      {
        type: 'p',
        text: 'The use of the SHA256 digest (@sha256:...) is mandatory for high-assurance signing. Tags are mutable pointers; signing a tag introduces a race condition (Time-of-Check to Time-ofUse) where the tag could be moved to a malicious image between the time of build and the time of signing.3 CleanStart images are essentially "distroless" or "minimal," lacking shells and package managers in production, which aligns with the philosophy of minimal trusted computing bases.4',
      },
      { type: 'heading', text: '1.3 Tooling Installation (Cosign)' },
      {
        type: 'p',
        text: 'The cosign CLI is the primary interface for Sigstore. It manages the OIDC handshake, keygeneration, and registry interaction.Installation for Linux (AMD64):',
      },
      {
        type: 'p',
        text: 'Verification:Verify the installation and ensure version consistency (v2.0+ is required for the flags detailedin this guide).',
      },
      {
        type: 'p',
        text: 'Note: Cosign v2 introduced breaking changes to the command-line flags, specifically regarding how identity verification is handled (--certificate-identity vs the older CS_ environment variables). This guide utilizes the v2.0+ syntax.5',
      },
      { type: 'heading', text: 'Step 2: Configure OIDC Identity Federation' },
      {
        type: 'p',
        text: 'The critical mechanism in keyless signing is the exchange of a workload identity token for a signing certificate. This requires configuring the Continuous Integration (CI) provider to mint OIDC tokens with the specific audience (aud) required by Sigstore.',
      },
      { type: 'heading', text: '2.1 GitHub Actions OIDC Configuration' },
      {
        type: 'p',
        text: "GitHub Actions functions as an OIDC provider. When a workflow runs, GitHub can issue a JSON Web Token (JWT) that attests to the workflow's identity. However, for security reasons, this capability is disabled by default and must be explicitly granted via permission scopes.The id-token: write Permission:To enable the OIDC projection, the workflow YAML must include the permissions block. This allows the GitHub runner to interact with the GitHub OIDC provider (token.actions.githubusercontent.com).Configuration (.github/workflows/release.yaml):",
      },
      { type: 'heading', text: 'Mechanism of Action:' },
      {
        type: 'p',
        text: 'When id-token: write is set, the GitHub Actions runner injects two environment variables:',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'Cosign detects these variables automatically. When cosign sign is invoked, it calls the REQUEST_URL specifying the audience as sigstore. GitHub responds with a JWT signed by its private key.',
      },
      { type: 'heading', text: 'Token Payload Structure (GitHub):' },
      {
        type: 'p',
        text: 'The JWT issued by GitHub contains specific claims that Fulcio will use to construct the certificate subject:',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'Fulcio uses the job_workflow_ref to create the Subject Alternative Name (SAN) in the certificate, which allows for precise verification later.8',
      },
      { type: 'heading', text: '2.2 GitLab CI/CD OIDC Configuration' },
      {
        type: 'p',
        text: 'GitLab CI/CD utilizes a specific keyword, id_tokens, to generate OIDC tokens. Unlike GitHub, which makes the token available via an internal endpoint, GitLab injects the token directly as an environment variable. Configuration (.gitlab-ci.yml):',
      },
      {
        type: 'p',
        text: 'Mechanism of Action: The id_tokens block instructs GitLab to generate a JWT with the audience sigstore and bind it to the environment variable SIGSTORE_ID_TOKEN.10 Cosign looks for this specific variable name by default. If you name it differently, you must pass it to Cosign manually, but standardizing on SIGSTORE_ID_TOKEN reduces configuration drift.',
      },
      ...
      {
        type: 'p',
        text: 'Fulcio maps these claims to the certificate extensions, allowing downstream users to verify that the image was built by a specific project and branch on GitLab.10',
      },
      { type: 'heading', text: 'Step 3: Execute Keyless Signing' },
      {
        type: 'p',
        text: 'Once the environment is configured and the OIDC token is accessible, the signing process can be executed. This step involves generating the image, pushing it to the registry, and then attaching the cryptographic signature.',
      },
      { type: 'heading', text: '3.1 Build and Push the Artifact' },
      {
        type: 'p',
        text: 'The artifact must exist in the registry before it can be signed because Cosign attaches the signature as a related artifact (stored via the OCI Reference specification).GitHub Actions Example:',
      },
      {
        type: 'p',
        text: 'The Importance of Digest Capture: The docker/build-push-action outputs the specific digest of the image built. This digest (steps.build-push.outputs.digest) is the immutable identifier. Signing the tag is considered a security anti-pattern because the tag can be moved to a different digest after verification but before deployment.3',
      },
      { type: 'heading', text: '3.2 The cosign sign Command' },
      {
        type: 'p',
        text: 'The signing command interacts with the Fulcio CA and Rekor transparency log.Command Syntax:',
      },
      ...
      {
        type: 'p',
        text: 'This third step is crucial. If you only verify the signature, you are confirming that someone with a valid OIDC token signed it. That "someone" could be a developer in a personal fork of the repository. You must verify that the specific trusted workflow signed it.',
      },
      { type: 'heading', text: '4.2 Constructing the Verification Command' },
      {
        type: 'p',
        text: 'The cosign verify command requires specific flags to define the trust policy.Verification Command Template:',
      },
      { type: 'p', text: 'Example for GitHub Actions:' },
      { type: 'heading', text: 'Identity String Precision:' },
      {
        type: 'p',
        text: 'The certificate-identity must match the SAN in the certificate exactly. For GitHub, this includes the repository path, the workflow filename, and the git reference (branch or tag).',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'Regex Matching:For flexibility (e.g., allowing signing from any tag starting with v), use the regex flags:',
      },
      {
        type: 'p',
        text: 'Warning: Regex usage requires careful construction to avoid "wildcard" vulnerabilities where an attacker could bypass the check using a similarly named branch.13',
      },
      { type: 'heading', text: '4.3 Policy Enforcement in Kubernetes' },
      {
        type: 'p',
        text: 'To enforce CleanStart standards, verification should be automated via an Admission Controller. Kyverno is a policy engine designed for Kubernetes that integrates natively with Cosign.',
      },
      { type: 'heading', text: 'Kyverno Policy for CleanStart Images:' },
      {
        type: 'p',
        text: 'This policy ensures that any Pod attempting to run an image from the cleanstart-corp repository must be signed by the specific GitHub Action workflow.',
      },
      {
        type: 'p',
        text: 'This policy acts as a firewall. If a developer attempts to deploy an image that was built locally (and thus unsigned) or built on a feature branch (if the policy enforces main), the cluster will reject the deployment request.',
      },
      ...
      {
        type: 'p',
        text: 'By verifying the base image, you establish a "chain of trust" extending from the OS vendor (CleanStart) to your application logic.',
      },
      { type: 'heading', text: '5.2 Offline and Air-Gapped Verification' },
      {
        type: 'p',
        text: 'CleanStart environments often operate in air-gapped networks where contacting the public Rekor log (rekor.sigstore.dev) is impossible. Sigstore supports "offline" verification using the Signed Certificate Timestamp (SCT) and the signature bundle.Procedure:',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'In offline mode, Cosign verifies the cryptographic signature and the validity of the certificate chain against the embedded Fulcio root, but it cannot query Rekor to see if the certificate was transparently logged. To mitigate this, Cosign relies on the SCT embedded in the certificate, which acts as a "promise" of logging.15',
      },
      { type: 'heading', text: 'Identity String Precision:' },
      {
        type: 'p',
        text: 'The public nature of Rekor allows for powerful auditing capabilities. You can query the log to see all artifacts signed by your identity. Audit Command:',
      },
      {
        type: 'p',
        text: 'If you detect a signing event that does not correlate with your known CI/CD build timestamps, it indicates a potential compromise of your OIDC provider or CI system.',
      },
      ...
      {
        type: 'p',
        text: 'To implement the solution described in this article, the following prerequisites must be met:',
      },
      { type: 'p', text: '\u25cf Cosign CLI: Version 2.2.0 or later installed on build agents.' },
      {
        type: 'p',
        text: '\u25cf CI/CD Platform: GitHub Actions, GitLab CI, or Google Cloud Build with OIDC enabled.',
      },
      {
        type: 'p',
        text: '\u25cf Container Registry: OCI v1.0 compliant registry with write access.',
      },
      ...
      {
        type: 'p',
        text: '\u25cb The OIDC provider URL (e.g., token.actions.githubusercontent.com).',
      },
      ...
      {
        type: 'p',
        text: '"FinTechGlobal," a provider of payment gateway infrastructure, faced a critical security challenge. Their compliance requirements (PCI-DSS and SOC 2) mandated strict provenance for all container workloads. They previously used long-lived GPG keys stored in Jenkins secrets. However, a near-miss incident where a key was inadvertently logged in a build artifact forced a re-evaluation of their strategy.',
      },
      { type: 'heading', text: 'The Implementation:' },
      {
        type: 'p',
        text: 'FinTechGlobal adopted CleanStart base images to eliminate OS-level CVE remediation work and implemented Sigstore keyless signing to secure the build pipeline.The Workflow:',
      },
      {
        type: 'ul',
        items: [
  ...
  'automated-attestation': {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'SLSA (Supply-chain Levels for Software Artifacts) defines maturity levels for supply chain security:',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'Test attestations prove security scans passed and tests succeeded before deployment.',
      },
      { type: 'heading', text: 'Implementation example' },
      { type: 'heading', text: 'Adding VEX documents' },
      {
        type: 'p',
        text: "VEX (Vulnerability Exploitability eXchange) documents explain why specific vulnerabilities don't affect your application:",
      },
      { type: 'heading', text: 'Stage 2: Build provenance attestation' },
      {
        type: 'p',
        text: 'Build provenance documents the build environment and ensures reproducibility.',
      },
      { type: 'heading', text: 'Implementation example' },
      ...
      {
        type: 'p',
        text: 'Source attestation proves which commit was used and that code review occurred.',
      },
      { type: 'heading', text: 'Implementation example' },
      ...
      {
        type: 'p',
        text: 'SBOM (Software Bill of Materials) attestations provide a complete inventory of components and their licenses.',
      },
      { type: 'heading', text: 'Implementation example' },
      ...
      {
        type: 'p',
        text: 'Use Open Policy Agent to enforce attestation requirements automatically:',
      },
      { type: 'heading', text: 'Kubernetes admission control with Kyverno' },
      {
        type: 'p',
        text: 'Enforce attestation policies at the Kubernetes admission control layer:',
      },
  ...
  'attestation-based': {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'Signing a container image proves one thing \u2014 that someone with a specific key signed it. It does not prove:',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: "Software artifacts are generally opaque blobs that can't easily be inspected for safety, so it's more common to reason about how they came to be rather than what is in them. We can't apply policy to individual lines of code, we apply policy on who built the software, how they built it, and where the code came from. This is the gap attestations fill. An attestation is a signed statement that captures specific metadata about an artifact \u2014 its provenance, its dependencies, its scan results.",
      },
      { type: 'heading', text: 'What is an attestation' },
      {
        type: 'p',
        text: 'A software attestation is an authenticated statement (metadata) about a software artifact or collection of software artifacts. The primary intended use case is to feed into automated policy engines. An attestation has three layers:',
      },
      { type: 'p', text: 'Layer' },
      { type: 'p', text: 'Purpose' },
      ...
      {
        type: 'p',
        text: 'The predicate is the metadata associated with the artifact, as a JSON object. The content of this object is derived from the value of the predicateType field. This might include the provenance (description of the origins of the artifact), SBOM (description of the contents and dependencies of the artifact), security checks such as SAST/DAST, and more.',
      },
      { type: 'heading', text: 'What In-Toto provides' },
      {
        type: 'p',
        text: 'In-toto provides a framework to protect the integrity of the software supply chain. It does so by verifying that each task in the chain is carried out as planned, by authorized personnel only, and that the product is not tampered with in transit. In-Toto defines:',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'The policy rule check fails if the signature is not found in the OCI registry, or if the image was not signed using the specified key. The rule mutates matching images to add the image digest, when mutateDigest is set to true, if the digest is not already specified.',
      },
      ...
      {
        type: 'p',
        text: 'Evidence that a specific supply chain step executed with specific inputs/outputs',
      },
      {
        type: 'p',
        text: 'The vulnerability predicate can be used to verify the compliance of the artifact against the company Security Gate. The test-result predicate can be used to verify the compliance of the artifact against the company Quality Gate.',
      },
      ...
      {
        type: 'p',
        text: 'Each failure type points to a different remediation path \u2014 missing signatures require pipeline changes, while condition failures may indicate policy misconfiguration or legitimate policy violations.',
      },
  ...
  'generate-slsa': {
    ...
    blocks: [
      ...
      {
        type: 'heading',
        text: "2. Call from Main Workflow: The developer's workflow calls the trusted builder.",
      },
      ...
      {
        type: 'heading',
        text: 'Step 4: Implement SLSA Level 3 Provenance (Method B: OpenSSF Generator)',
      },
      {
        type: 'p',
      ...
      {
        type: 'p',
        text: 'The "Generic Generator" is ideal for arbitrary binaries or artifacts where a language-specific builder (like the Go builder) is not feasible.',
      },
      {
        type: 'ol',
        items: [
      ...
      {
        type: 'p',
        text: 'If building a container image, use the generator_container_slsa3.yml workflow. This workflow pushes the attestation to the container registry, attaching it to the image manifest. YAML',
      },
      { type: 'heading', text: 'Step 5: Verify Provenance (The Consumer)' },
      {
        type: 'p',
        text: 'Provenance generation is futile without verification. Consumers (deployment systems, users, or downstream builds) must verify the attestation to confirm the artifact is authentic and meets SLSA Level 3 standards.',
      },
      { type: 'heading', text: 'Action: Verify using CLI Tools' },
      {
        type: 'p',
        text: 'Verifying Native Attestations (Method A) Use the GitHub CLI (gh) to verify artifacts signed by the attest-build-provenance action.',
      },
      ...
      {
        type: 'p',
        text: 'Use the slsa-verifier tool for artifacts generated by the OpenSSF workflows. This tool provides granular control over the expected builder identity. Bash',
      },
      {
        type: 'p',
        text: 'Deep Insight - The "Source" Check: The --source-uri flag is the most critical security control here. It prevents a "forking attack" where an attacker forks your repository, runs the valid SLSA generator in their fork (producing valid provenance signed by their workflow), and presents the malicious artifact + valid provenance to a user. slsa-verifier checks that the OIDC identity specifically matches the expected source repository, rejecting valid provenance from unauthorized sources.13',
      },
      { type: 'heading', text: 'Step 6: Policy Enforcement and Automation' },
      {
        type: 'p',
        text: 'For enterprise environments, manual verification is insufficient. SLSA verification should be automated via admission controllers or policy engines.',
      },
      { type: 'heading', text: 'Kubernetes Admission Control (Kyverno/Gatekeeper)' },
      {
        type: 'p',
        text: 'Deploy policies that block any container image lacking a valid SLSA Level 3 attestation from the authorized organization.',
      },
      {
        type: 'ul',
        items: ['CleanStart Integration: Enhance this policy to require two attestations:'],
      },
      {
        type: 'ol',
        items: [
      ...
      {
        type: 'ol',
        items: ['The Application layer attestation (verifying your code was built securely).'],
      },
      {
        type: 'p',
        text: 'This "chained verification" ensures that a secure build process wasn\'t wasted on a compromised foundation.14',
      },
      { type: 'heading', text: 'Notes / Tips' },
      {
        type: 'ul',
      ...
      {
        type: 'p',
        text: "This section provides deep research insights into why the above steps constitute a robust security posture, analyzing the interplay between SLSA standards and CleanStart's upstream philosophy.",
      },
      { type: 'heading', text: 'The Mechanics of Isolation' },
      {
        type: 'p',
        text: 'SLSA Level 3\'s defining characteristic is Isolation. In a non-isolated build (Level 2 or lower), a build script might have access to secrets used for signing, or it might run on a machine ("runner") that has persisted files from a previous build.',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'Most SLSA tutorials focus solely on the build process. However, a SLSA Level 3 build that compiles malware is still a valid SLSA Level 3 build\u2014it just accurately proves that malware was built securely.',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'Best for enterprises already deep in the GitHub ecosystem requiring privacy.',
      },
      {
        type: 'p',
        text: 'Best for open-source projects wanting distinct, community-standard verification.',
      },
      { type: 'heading', text: 'Stage 3: Test and security attestation' },
      {
        type: 'p',
      ...
      {
        type: 'p',
        text: 'The integration of SLSA Level 3 provenance with automated policy enforcement represents the future of DevSecOps. We are moving away from implicit trust ("I trust this developer") to explicit, cryptographic verification ("I trust this artifact because it was built by this specific workflow, on this platform, from this commit, on this verified base image"). CleanStart\'s approach of providing "pre-verified" components aligns perfectly with this future. Instead of every organization spending resources hardening the same Alpine or Ubuntu images, they consume a verified upstream artifact, chaining their provenance to it. This reduces the global "computational waste" of security patching and accelerates the adoption of secure-by-default pipelines.1',
      },
      ...
      {
        type: 'p',
        text: 'This structured approach ensures that your organization not only "checks the box" for SLSA compliance but fundamentally elevates the security posture of its software delivery, mitigating risks from the source code all the way to the production runtime.',
      },
  ...
  'runtime-evidence': {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'eBPF (extended Berkeley Packet Filter) runs sandboxed programs in the Linux kernel without modifying kernel source code or loading kernel modules. This provides kernel-level visibility into container behavior with minimal performance overhead.',
      },
      ...
      {
        type: 'p',
        text: 'Before implementing eBPF monitoring, verify your environment meets these requirements:',
      },
      ...
      {
        type: 'p',
        text: 'Cilium provides network observability and policy enforcement using eBPF.',
      },
      ...
      {
        type: 'p',
        text: 'Tetragon provides flexible security observability with custom tracing policies.',
      },
      ...
      {
        type: 'ul',
        items: ['/sys filesystem', '/proc filesystem', 'Kernel BTF', 'Kernel modules directory'],
      },
      ...
      {
        type: 'ul',
        items: ['Observe events without taking action', 'Establish baselines for normal behavior'],
      },
      ...
      {
        type: 'ul',
        items: ['<1% CPU idle', '2-5% CPU under active monitoring', '100-300 MB memory per node'],
      },
      {
        type: 'p',
        text: 'If overhead exceeds this, investigate event rates and consider sampling.',
      },
      { type: 'heading', text: 'Related Articles' },
      {
        type: 'ul',
