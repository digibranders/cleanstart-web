...
const ATTRIBUTED_COLLECTIONS: readonly CountedCollection[] = [
  { slug: 'blogs', label: 'blog post' },
  { slug: 'news', label: 'news item' },
  { slug: 'guides', label: 'guide' },
  { slug: 'knowledgeBase', label: 'KB article' },
];
      ...
      try {
        const results = await Promise.all(
          ATTRIBUTED_COLLECTIONS.map(async (entry) => {
            ...
            url.searchParams.set('where[authors][in]', String(id));
        ...
        for (const r of results) {
          total += r.totalDocs ?? 0;
          const candidate = r.docs?.[0]?.updatedAt;
