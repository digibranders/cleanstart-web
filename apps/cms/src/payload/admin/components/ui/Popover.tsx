...
type Props = {
  ...
  /** Role for the floating element. Default `'dialog'`. Use `'menu'` for menu-style popovers. */
  readonly role?: 'dialog' | 'menu' | 'listbox';
  /**
   * Match the floating element's width to the anchor. Useful for
   * combobox-style popovers where the dropdown should line up flush with
   * the input that spawned it.
   */
...
export const Popover = (props: Props): ReactElement | null => {
  ...
  const anchorWidth =
    matchAnchorWidth && anchorRef.current ? anchorRef.current.getBoundingClientRect().width : null;
