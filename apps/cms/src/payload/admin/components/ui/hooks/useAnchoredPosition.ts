 ...
 * (cheap; only runs while the floating element is visible).
 *
 * Hand-rolled rather than pulling Floating UI — our placements are simple,
 * we want zero new deps, and the existing custom popovers (LinkPopover,
 * TableGridPicker) already do this kind of math inline. Centralising here
 * means the next ten popovers don't repeat it.
 */
export const useAnchoredPosition = (args: Args): Position | null => {
  const { anchor, floating, open, placement = 'bottom-start', offset = 6, viewportPadding = 8 } = args;
