...
| 14 | SaaS | `/industries/saas-container-security` | Static | ✅ | Title, description and H1 are the SEO team's, applied verbatim. **Renamed from `/saas` 2026-08-31**; that path returned 404 in production, so no redirect was needed (unlike its sibling, which resolved). **Live 2026-09-02**: indexable (the `noindex,nofollow` pair dropped), listed in `STATIC_ROUTES` and nav-linked (Solutions › By industry). Breadcrumb, `JsonLdGraph` and `pageRegistry` row (id=43) in place. |
