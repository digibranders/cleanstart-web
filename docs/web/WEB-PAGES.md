...
| 13 | Financial Services | `/financial-services` | Static | ✅ | Built from the client proposal docx. **Live and indexable** as of client sign-off: `index,follow`, in the sitemap's `STATIC_ROUTES`, nav-linked (Solutions › By industry), breadcrumb + `JsonLdGraph` in place, 6h ISR. Outstanding: no `pageRegistry` row in the CMS, so the graph has no `WebPage` node (every sibling page has one) — add the row to complete it. |
| 14 | SaaS | `/saas` | Static | 🚧 | Built from the client proposal docx. Softer launch than Financial Services: **not** nav-linked, `noindex,nofollow`, no sitemap entry, no JSON-LD graph, so it is reachable by direct URL only. Slug is cheap to change while unindexed and expensive after. |
