...
| 14 | SaaS | `/industries/saas-container-security` | Static | 🚧 | Title, description and H1 are the SEO team's, applied verbatim. **Renamed from `/saas` 2026-08-31**; that path returned 404 in production, so no redirect was needed (unlike its sibling, which resolved). `noindex,nofollow` and out of the sitemap pending copy approval. Nav-linked (Solutions › By industry). Still missing the breadcrumb + `JsonLdGraph` pair and a `pageRegistry` row — add both before launch so it emits the same graph as every other solutions page. |
