...
| 8d | Tricorder | `/tricorder` | Static | ✅ | Built 2026-09-16 from the "The Intelligence Layer" copy doc (no Figma; every scene is drawn in SVG/CSS on the site's tokens). Sections: `TricorderHero` (scan-console artifact on a radar sweep) → `TricorderThreatGap` ("Not Every Threat Has a CVE", three evidence cards: version timeline / behaviour diff / relationship graph) → `TricorderContext` ("Three Ways to Read Software Components", the copy doc's own diagram, rebuilt in the site's language: the three signal groups (History / Behavior / Relationships) each showing what they read (the 1.4.1→1.4.2→1.4.3 version chain; Network / Shell / File System / Code; Maintainer → Packages → Infrastructure → Endpoints), their currents converging into the `COMPONENT package-name@1.4.3` card, where the diagram ends. The `TRICORDER VERDICT` card was removed on 2026-09-22: `TricorderPipeline` directly below already ends on the verdict, so carrying it here resolved the same story twice. Two deliberate translations from the doc's mock: glass panels on the dark gradient instead of a white page (the page's light/dark alternation depends on this section staying dark), and the site's teal in place of the doc's green, which is not in the palette. Fixed 1320 × 402 canvas via `ScaleToFit`; stacks below lg. Heading is left-aligned so the section still reads differently from the centre-headed sections either side of it) → `TricorderPipeline` (Analyze → Compare → Correlate → Enrich → Verdict terminal) → `TricorderSubstrate` (the three products on glass pedestals with the shared hexagonal product art from `cleanstart-factory/`, currents dropping into a perspective "intelligence layer" floor that carries the Tricorder emblem and the four stages; headed "One Intelligence Layer. Three Decision Points."; cards run Clean Libraries → Clean Images → CleanSight, the order the sub-head names and the order a component meets them in life (build, release, fleet), reordered 2026-09-22 from the older Images-first order; each card is **titled** by its decision point (`Build-time` / `Release-time` / `Fleet-level`): that is the card's `h3`, at `--fs-h4` in white with its glyph beside it and the link arrow opposite, and the product name drops under it as a tinted `--fs-body` line above the blurb. The section outline therefore reads H2 "Three Decision Points" into three decision-point H3s. Do not restore the product name to the title slot. The footer bar and its hairline were removed 2026-09-22 because the H2 promises three decision points and the cards were burying them. The labels are deliberately short: the H2 carries the word "decision" once, so repeating it on two of three chips (and breaking the set with "intelligence" on the third) was noise. Do not re-lengthen them without re-reading the H2; `PRODUCTS` order drives the cards, the landing dots and the branch gradient together, so that array is the one place to change it; anchor `#one-intelligence-layer`, hardcoded and unaffected by the heading rewrite) + `TricorderCTA` in the footer slot ("Talk to an Expert" → `/contact-us`). Emits BreadcrumbList + SoftwareApplication. **Copy fidelity:** every headline, sub-head and body line is the copy doc's verbatim, including the four stage names and their descriptions and the `Analyze → Compare → Correlate → Enrich → Verdict` flow arrows. The `TricorderPipeline` sub-head and all four stage descriptions were rewritten from the doc on 2026-09-22, so check the revision date before restoring older wording; the product-card and lens lines come from the doc's own embedded mockups. "Talk to an Expert" is the only CTA the doc specifies, so it is the only one the page carries. Everything else rendered inside the visualisations (package names, versions, findings, code lines, graph nodes, the CTA verdict ledger) is **illustrative sample data, not product claims** — replace it if product supplies real examples. Indexable and in `STATIC_ROUTES` (the `noindex,nofollow` hold and the nav removal of 2026-09-18 were both lifted on 2026-09-22); linked from Products nav (`lens` glyph, last in the Products group because Tricorder is the layer the other three run on), the footer Product column and `llms.txt`. SERP title is `Malicious Package Detection | Tricorder by CleanStart` and the meta description leads on malicious/tampered components passing CVE scans: the page's H1 and OG card keep the brand line ("The Intelligence Layer for Software Trust"), the `<title>` targets the query, same split as `/cleansight`. **Still open: add a `pageRegistry` row for `/tricorder` in the CMS** (`webPageType: WebPage`, title `Tricorder`): every sibling product page has one, and without it `getPageGraph` emits no WebPage node, so the `primaryImageOfPage` the page already passes (`/images/tricorder/hub-intelligence-cube.webp`) is dropped on the floor. **Also open: the page is ~240 words of body copy.** Every ranking signal below is in place, but that is too thin to compete on a commercial term; it needs a real prose or FAQ section before it will rank for anything but the brand. |
