...
| 13 | Financial Services | `/financial-services` | Static | 🚧 | Built from the client proposal docx. Linked from the nav (Solutions › By industry) but still `noindex,nofollow`, absent from the sitemap and carrying no JSON-LD graph — reachable by people, not by search. Lift the flags and add the breadcrumb/JsonLdGraph pair on client sign-off. |
