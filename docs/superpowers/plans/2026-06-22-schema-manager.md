...
- **INV-3 (lossless migration):** Phase 0 MUST NOT change the *semantic schema content* (the set of nodes and their properties) for any existing page type, except where a Phase 4 consistency fix is explicitly intended. Note: Task 0.5 deliberately changes the *structure* from N separate `<script>` blocks to ONE connected `@graph` (per-blob `@context` stripped, nodes de-duped by `@id`) — this is the single sanctioned structural change of Phase 0 and is semantically equivalent for consumers. Verified by node-set snapshot tests, not raw byte diffs.
