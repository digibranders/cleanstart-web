...
| Cube face branding | **Tech logos on cube faces** — reuses existing `/images/hero-tech-logos/*.svg` (nginx, postgres, redis, kafka, mongodb, python, node, mysql, prometheus, grafana, …) |
...
#### 3.4.1 · Cube face logo (recurring brand element)

Every cube carries a real upstream-tech logo on a billboarded `Plane` mounted just in front of the cube body. The plane is camera-facing (always readable regardless of the cube's own rotation), at `0.72 × 0.72` units, with the logo SVG rendered into a texture at 2× DPR.

**Asset source:** the existing `apps/web/public/images/hero-tech-logos/` directory. The 16 existing SVGs (`nginx`, `postgres`, `redis`, `kafka`, `mongodb`, `python`, `node`, `mysql`, `helm`, `prometheus`, `grafana`, `vault-k8s`, `envoy`, `consul`, `etcd`, `keycloak`) are the eligible pool. **Do not introduce new logos for v1** — re-use what's already there.

**Selection per cube:** round-robin from a curated subset (the 10 most-recognized: `nginx`, `postgres`, `redis`, `python`, `node`, `mongodb`, `kafka`, `mysql`, `prometheus`, `grafana`). Round-robin deterministic so the same seed produces the same sequence (testable). New spawn each 5 s = ~12 different logos cycled per minute.

**Logo material state:**

| Cube state | Logo treatment |
|---|---|
| Dirty (CH1 → mid-CH2) | Desaturated to grayscale; tint multiplied by `#5a4a78` at 0.75 alpha; sits behind the CVE decals which orbit *on top* of the logo plane |
| In-transit (CH2 → CH3) | Smooth grayscale-→-color crossfade over 1.2 s during CleanCompile (4.4s → 5.6s window) |
| Clean (post-CH3 → exit) | Full saturation; a soft `--neon-primary` glow halo (radius `0.4` units, `α 0.35`) blooms around the logo plane on `BloomLayer = 1` |

**Logo legibility constraints:**

- Minimum rendered size on screen at desktop 1440 px: **48 px**. If a chamber-zoom causes the cube to scale below this, logo opacity is suppressed until it returns to the rail.
- Logos are decorative — they have no `aria-label`. The hero's overall `aria-label` (§ 7) mentions "real upstream container images" without enumerating them.
- Each SVG is sanitized and inlined as a single `<symbol>` in the canvas's texture atlas — no individual fetch per cube.

**Implication on § 3.6:** the CH1 manifest card no longer lists package names (the logo already says "postgres"). Instead, it now shows a CVE-summary chip: `🔴 3 CVE · v14.2 · 247 deps`. Keeps the data-viz layer purposeful, removes redundancy with the cube logo.
...
|---|---|---|
| 01 · Intake | A small floating CVE-summary chip near the cube: `🔴 3 CVE · v14.2 · 247 deps` (numbers vary per logo via a static lookup table). Fades on cube exit. | 0.5 s → 1.8 s |
...
| 03 · CleanCompile | Cube dematerialises into a wireframe build-lattice. Lattice grows layer-by-layer from bottom (3 layers, staggered 200 ms). Cube re-materialises clean. Logo crossfades grayscale→color during this window. | 4.4 s → 5.9 s |
| 04 · Attest · Handoff | A signature seal stamps onto the cube's face (next to the logo). An SBOM "ticker tape" prints down from a small printer mounted on the chamber ceiling — 7 rows of micro-text, each row a hash fragment. | 6.5 s → 8.0 s |
...
└── factory-hero/
    ...
    │   ├── Cube.tsx                  (artifact cube, dirty + clean material variants, accepts logoSrc prop)
    │   ├── CubeLogoPlane.tsx         (billboarded logo plane, grayscale→color shader)
    ...
    │   └── logoPool.ts               (curated logo round-robin: 10 SVGs from /images/hero-tech-logos/, with per-logo CVE-summary lookup for CH1 chip)
    └── assets/
        ├── hero-chamber.glb          (single shared chamber mesh)
        └── hero-poster.jpg           (LCP poster)

    /* External assets (not under this folder — re-used from existing repo): */
    apps/web/public/images/hero-tech-logos/
        ├── nginx.svg, postgres.svg, redis.svg, python.svg, node.svg,
        ├── mongodb.svg, kafka.svg, mysql.svg, prometheus.svg, grafana.svg
        └── (+6 unused-by-hero: helm, vault-k8s, envoy, consul, etcd, keycloak)
```
...
- The hero is `role="img"` with `aria-label="CleanStart Factory pipeline: vulnerable upstream container images (such as nginx, postgres, redis) enter on the left, pass through four hardening stages — Intake, AI Logic Engine, CleanCompile, Attest and Handoff — and exit signed, verified, and CVE-free on the right."`
...
9. Bundle delta for the home route ≤ 220 KB gzipped vs. baseline. *(The 10 logo SVGs already ship today as part of the orb; reuse is byte-neutral.)*
10. No imports added from `@payloadcms/ui` or other CMS-side packages.
11. Every cube carries a recognizable tech logo (`nginx`/`postgres`/`redis`/etc.) on its camera-facing front face, grayscale + dim while dirty, color + glowing while clean, with a smooth crossfade during the CleanCompile dwell. Logo rotates per round-robin from `logoPool.ts`.
