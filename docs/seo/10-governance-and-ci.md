...
- **Evidence:** `packages/schema/src/compose/compose-graph.test.ts` (10 cases, including its own "INV-5" invariants) is the closest existing analogue in this codebase, exercising graph-composition determinism and merge behavior against fixture input — but a test explicitly asserting CMS-field-to-JSON-LD-field equality was not identified in this pass, and per GOV-02 this file does not run in CI regardless.
- **CleanStart:** Unverified — no test explicitly asserting CMS-field-to-JSON-LD-field equality was identified in the audited evidence; `compose-graph.test.ts` covers composition/merge behavior, not source-field equality specifically, and a dedicated search for this exact check was not performed in this pass
...
- **Evidence:** This pass did not re-audit every collection's field-level `required` configuration against the publishing checklist's own list of SEO-required fields.
- **CleanStart:** Unverified — collection-by-collection `required: true` coverage against the publishing checklist's own field list was not re-audited in this pass
