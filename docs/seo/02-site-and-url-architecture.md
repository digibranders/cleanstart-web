...
- **CleanStart:** Unverified — no crawl of nav/footer/mega-menu/hub-page markup was performed in this evidence pass

  The one internal-link mechanism directly inspected for this module — the shared `breadcrumbTrail()`/`HeroBreadcrumb` path — does render real `<a href>` elements, but a single mechanism's conformance does not stand in for a sitewide internal-linking audit. Re-run the Verify command across a representative page sample (nav, footer, category hubs, in-body links) before recording a Pass or Fail.
...
- **CleanStart:** Unverified — no homepage-seeded crawl was run against the live site to diff against the sitemap's URL list

  `docs/seo/evidence/codebase-inventory.md` independently flags the same class of open question under "URL Architecture & Sitemaps": whether `/pricing`'s sitemap inclusion reflects current intent versus a stale comment (`sitemap.ts:71` calls it "intentionally omitted," but code and the built page both include it — likely just a stale comment, not an orphan risk), and whether `SECTION_INDEX_REDIRECTS`' hardcoded targets for `/knowledge-hub` and `/legal` could silently diverge from what a live CMS-ordering-driven crawl would compute (see ARCH-13). Neither has been resolved by an actual crawl in this evidence pass.
...
- **Reference:** `apps/web/src/lib/seo/canonical.ts:186-206` (`buildListingMetadata`, always canonicalizes to a clean `basePath`); `apps/web/src/app/sitemap.ts:135-157` (per-document detail-page URLs, independent of the listing pagination)
...
- **CleanStart:** Unverified — no click-depth distribution was measured against the live site in this evidence pass

  This rule is a navigation-design principle rather than a single mechanical check; confirming conformance requires the same homepage-seeded crawl referenced in ARCH-04, which was not run for this module.
