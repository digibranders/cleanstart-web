...
- **Verify:** Manual — `apps/web` is Vercel-hosted and produces no `access.log`; no log drain is currently wired for this site (the Caddy `access.log` referenced elsewhere in this repo belongs to the `cms.cleanstart.com` droplet, a different host, and doesn't see marketing-site traffic). Until a Vercel log drain (e.g. BetterStack/Axiom) is provisioned, check Search Console's Crawl Stats report (by file type/response) for repeated crawler hits on disallowed facet-parameter paths instead of grepping a log that doesn't exist.
