...
- **Evidence:** `robots.ts:32-34` documents the intended enforcement mechanism in a comment: "the disallow here is symbolic — it is backed by a Vercel Firewall rule matching on User-Agent." That Vercel Firewall rule is referenced in `docs/web/WEB-PRODUCTION.md:112-113` ("Block requests where `User-Agent` matches `/Bytespider/i` → action: `deny` (HTTP 403)") and is not present anywhere in this repository, but this rule's own `Verify` command is directly testable against the live site with no dashboard access required, and running it settles the question: `curl -A "Bytespider" -I https://www.cleanstart.com/` returns `403` with `x-vercel-mitigated: deny`, confirmed live 2026-07-29 — while `curl -A "GPTBot"` and `curl -A "ClaudeBot"` against the same URL both return `200`. The firewall-level enforcement this rule requires is active in production; a plain GET demonstrates it without needing Vercel dashboard/API access.
- **CleanStart:** Pass
