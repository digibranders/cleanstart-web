...
- **Verify:** Manual — an editorial review, not a script: for a sample of generated templates, have an independent reviewer (not the page's builder) write down the specific non-ranking reason each page needs to exist. (This rule's Acceptance is a judgment call about purpose, not a content-similarity diff — `check-locale-content-similarity.mjs`, cited here in an earlier pass, does not exist and would not answer this question even if it did; that script name belongs to PROG-07's near-duplicate-diff check instead.)
...
- **Verify:** Manual — no `access.log` or crawl-log artifact exists for a Vercel-hosted client site by default; pull crawler-request-by-path volume from the hosting platform's log drain (if provisioned) or Search Console's Crawl Stats report, and compare against the crawl-control design doc's stated intended URL count.
- **Reference:** None — no reference implementation
...
- **Verify:** Manual — no `access.log` or crawl-log artifact exists for a Vercel-hosted client site by default; pull crawler-request-by-path volume from the hosting platform's log drain (if provisioned) or Search Console's Crawl Stats report, and compare disallowed-pattern volume (should trend to zero) against canonicalized-but-crawlable facet volume (persists by design).
- **Reference:** None — no reference implementation
...
- **Verify:** Manual — crawl the site with sitemap-seeding disabled (a pure link-graph crawl) and confirm no generated-template URL that appears in the sitemap is missing a real `<a href>` inbound link from a crawlable page. (This rule's Acceptance is about link-graph reachability, not hreflang — `check-hreflang-matrix.mjs`, cited here in an earlier pass, does not exist and is unrelated to this rule's own check; that name belongs to the INTL-04/05/06/16 cluster-completeness checks instead.)
- **Reference:** None — no reference implementation
...
- **Verify:** Manual — no automated script exists in this repo; diff the rendered HTML body content (excluding nav/footer/boilerplate) of a random sample of sibling pages from the same template, per this rule's own `Tools` field.
- **Reference:** None — no reference implementation
