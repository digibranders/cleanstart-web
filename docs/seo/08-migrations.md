...
- **Verify:** Manual — no automated script and no dated cutover log exist in this repo yet; before removing any redirect row, look up its `hitCount`/`lastHitAt` in the CMS admin, confirm ≥365 days have elapsed since its (undocumented) cutover date, and confirm zero hits in the trailing 30 days.
