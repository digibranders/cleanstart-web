...
- **What is actually dead is the CMS's own dispatcher** — `apps/cms/src/payload/lib/jsonld/dispatch.ts` (`buildJsonLdBlobs`, its three layers of auto/`schemaAddons`/`additionalSchema` composition), the `/api/jsonld` endpoint that exposes it, and the `schemaAddons` field attached to nine collections. The only callers of that endpoint anywhere in the repository are the CMS admin's own "Schema (JSON-LD)" preview sidebar and the CMS's own e2e tests — `apps/web` calls it zero times. The CMS's own code says so: `apps/cms/src/payload/endpoints/page-live-schema.ts:6-13` states that after a prior unification, "a page's auto schema... is composed in apps/web at build time — the CMS does not recompute it," and that the CMS instead fetches the rendered HTML of the live page and regex-extracts its `<script type="application/ld+json">` blocks to find out what actually shipped — the CMS had to build a scraper of its own output's destination because its dispatcher's output never reaches that destination.
- **`packages/schema`'s own test suite — 86 test blocks / 174 `expect()` calls across 7 files — never runs in any CI job.** No GitHub Actions workflow references `packages/schema`; `apps/web/vitest.config.ts` scopes its `include` to `apps/web/src/**` only, so a green `apps/web` test run says nothing about the shared package's own tests.
...
- **Verify:** `grep -rniE "(boost|improve|rank higher).{0,40}(structured data|schema|json-ld)" docs/ --exclude-dir=seo 2>/dev/null | wc -l` → `0` (excludes `docs/seo/` itself, since this rule's own text discusses "improve ranking"/"rank higher" while documenting the prohibition — a self-match there is not a violation)
...
Evidence: `/email-signatures` is `noindex, nofollow` and `Disallow`'d in `robots.txt` (per CRAWL-01/CRAWL-04), yet its live page carries `jsonLdTypes: ["Organization", "WebSite"]` (`live-capture.json`). Root cause: `Organization`/`WebSite` JSON-LD is emitted globally from `RootLayout` (`apps/web/src/app/layout.tsx:189`, `<JsonLdGraph id="site-jsonld" graph={siteGraph} />`) on every page regardless of that specific page's indexing status — it is not scoped per-route. Practical risk is low: this rule's acceptance criterion cares about content Google can actually crawl and mark up in Search, and Google cannot fetch a `Disallow`'d page to consume its structured data in the first place, so the exposure here is a hygiene gap (marking up a page that's meant to be fully suppressed), not a live rich-result or Search-appearance risk. Search Console Manual Actions report access was not confirmed and no scripted field-level markup-vs-DOM diff was run in this pass, so the rest of this rule's surface remains otherwise unverified.
...
- **Reference:** `packages/schema/src/builders/site.ts:7-8` (single `SITE_URL`/`SITE_NAME` source); `packages/schema/src/compose/compose-graph.ts:30-42` and `merge.ts:33-49` (dedupes by `@id`, last value wins, first position kept) — a structural guard against the two-entities-one-`@id` failure mode.
...
- **Reference:** `apps/web/src/lib/seo/seo-defaults.ts` → `orgConfigFromDefaults()`, consumed at `apps/web/src/app/layout.tsx:153`; hardcoded fallback in `organizationSchema()` (`packages/schema/src/builders/jsonld.tsx:151-212`) if the CMS is unreachable. `Organization` appears in `jsonLdTypes` on 57 of the 59 pages captured in `docs/seo/evidence/live-capture.json` (2026-07-29) — the other two, `sitemap.xml` and `llms.txt`, are non-HTML routes where JSON-LD does not apply, so this is full coverage of every route where the type is applicable, not a gap. `logo` dimensions and `sameAs` completeness **were** checked against live property values in this pass, correcting an earlier note that they weren't: production emits `"logo":{"width":459,"height":96}` — a confirmed defect, not an open question (see Evidence below).
...
Evidence: confirmed live 2026-07-29 — the homepage's `Organization` node emits `"logo":{"@type":"ImageObject","url":"https://www.cleanstart.com/images/cleanstart-logo.png","width":459,"height":96}`. `width` clears the 112×112px floor; `height` (96px) does not — a confirmed defect against this rule's own acceptance criterion, not an open question. `name`/`url`/`logo` are otherwise identical sitewide (single `orgConfigFromDefaults()`/hardcoded-fallback source), and no fabricated `foundingDate`/`vatID` was found.
...
- **Reference:** `docs/seo/evidence/live-capture.json` (2026-07-29 capture) confirms `JobPosting` on `job/[slug]` (e.g. `job/senior-software-engineer`); a verifier read every file in `apps/cms/src/payload/jobs/` (the project's own cron-job directory, cross-referenced against `CLAUDE.md`'s "Background jobs" table) and confirmed **no expiry-monitoring cron exists** — this is a stated absence, not an unlocated process.
...
- **Reference:** `docs/seo/evidence/live-capture.json` (2026-07-29 capture) shows `BreadcrumbList` present on every detail template and on the `blogs`, `careers`, `case-studies`, `guide`, `resource-center`, `knowledge-hub`, `podcast`, and `legal` listing pages, but absent from the `events`, `news`, and `webinars` listing templates.
...
- **Reference:** `apps/cms/src/payload/admin/components/SchemaPreviewField.tsx:214` validates against the CMS's own dispatcher (SCHEMA-03), not against the live page — do not mistake that preview passing for a live-page validation pass.
