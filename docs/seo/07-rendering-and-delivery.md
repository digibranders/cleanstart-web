...
- **Evidence:** `podcast/page.tsx:31-35`'s comment states the page's CMS fetch uses "revalidate 60," but no `revalidateSeconds` override was found in `lib/podcast.ts` in this pass — meaning the page's CMS reads plausibly fall through to `cms-fetch.ts`'s shared 3600s default instead of the 60s the comment claims; not traced to a definitive call site, so this remains an open discrepancy (`codebase-inventory.md` Contradictions §B5/§D). Separately, `/api/revalidate`'s Mode 2 auth branch (`route.ts:59-79`, including its `NAV_CACHE_TAGS` allow-list) has no in-repo caller — the CMS's only sender always uses Mode 1's Bearer-token shape, and a repo-wide search for `process.env.REVALIDATE_SECRET` found exactly one hit, the read site itself. An external caller (a manual request, an undiscovered script, an unmatched Payload hook) cannot be excluded by grep alone.
- **CleanStart:** Unverified — the podcast page's actual effective revalidation interval (60s per comment vs. 3600s shared default) was not traced to a definitive call site in this pass; the on-demand Mode 2 path's reachability from outside this repo cannot be excluded by static analysis
...
- **Verify:** `curl -sI -H "If-None-Match: \"$(curl -sI https://www.cleanstart.com/ | grep -i etag | cut -d'\"' -f2)\"" https://www.cleanstart.com/`
- **Reference:** None — no reference implementation (not traced whether Vercel's edge layer implements conditional-GET support for this deployment in this pass)
...
- **Evidence:** Not traced in this pass whether `apps/web`'s Vercel deployment honors `If-None-Match`/`If-Modified-Since` for prerendered pages; the live-capture headers reviewed did not include an `ETag` on the sampled pages.
- **CleanStart:** Unverified — conditional-GET support was not tested against a live request in this pass
