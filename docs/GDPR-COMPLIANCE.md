...
| Item | Status | Detail |
|---|---|---|
| **Cookie consent / CMP** | 🔴 Missing | No banner, no CMP library in `package.json`, no `/api/consent`, no `cs_consent` cookie, no Consent Mode. `docs/WEB-PRODUCTION.md` fully specifies it (geo-gated to EEA/UK/CH/CA, Consent Mode v2 with all 4 signals default-denied, one-click "Reject all" parity) — **none of it is built.** |
